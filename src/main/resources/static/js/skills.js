...
async function loadSkills() {
  try {
    ...
    document.querySelectorAll(".delete-skill-btn").forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        ...
        if (confirm("Yetenek silinsin mi?")) {
          try {
            const resDel = await fetch(`${baseUrl}/skill/admin/${delId}`, {
              method: "DELETE",
              headers: {
                Authorization: `Bearer ${token}`,
              },
            });
            const resData = await resDel.json();
            if (resData.result) {
    ...
    skillListDiv.innerHTML = `<p>Sunucuya bağlanılamadı: ${err.message}</p>`;
  }
}
...
skillForm.addEventListener("submit", async (e) => {
  ...
  const url = id ? `${baseUrl}/skill/admin/${id}` : `${baseUrl}/skill/admin`;
